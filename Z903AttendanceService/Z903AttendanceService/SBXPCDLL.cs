...
namespace sbxpc
{
    class SBXPCDLL
    {
        ...
        [DllImport(Dll32, EntryPoint = "_SetDeviceNumber", CallingConvention = CallingConvention.Winapi)]
        static extern byte _SetDeviceNumber_32(Int32 dwMachineNumber);
        [DllImport(Dll64, EntryPoint = "_SetDeviceNumber", CallingConvention = CallingConvention.Winapi)]
        static extern byte _SetDeviceNumber_64(Int32 dwMachineNumber);

        public static int SetDeviceNumber(Int32 dwMachineNumber)
        {
            try
            {
                return Environment.Is64BitProcess
                    ? _SetDeviceNumber_64(dwMachineNumber)
                    : _SetDeviceNumber_32(dwMachineNumber);
            }
            catch (Exception)
            {
                return -1;
        ...
        public static int ConnectTcpipStatus(Int32 dwMachineNumber, string lpszIPAddress, Int32 dwPortNumber, Int32 dwPassWord)
        {
            if (lpszIPAddress == null)
                return -1;
        ...
        /// <summary>
        /// Sets user in machine with name, privilege, enables user and device. Logs errors.
        /// </summary>
        public static bool SetUserInMachine(int dwMachineNumber, int dwEnrollNumber, string userName, int privilege, int backupNumber, int eMachineNumber)
        {
            // Set user name (mandatory)
            if (!SetUserName1(dwMachineNumber, dwEnrollNumber, userName))
            {
                int errCode;
                GetLastError(dwMachineNumber, out errCode);
                Console.WriteLine($"SetUserName1 failed for user {dwEnrollNumber} on machine {dwMachineNumber}. SDK Error Code: {errCode}");
                return false;
            }

            // Modify privilege (optional)
            if (!ModifyPrivilege(dwMachineNumber, dwEnrollNumber, eMachineNumber, backupNumber, privilege))
            {
                int errCode;
                GetLastError(dwMachineNumber, out errCode);
                Console.WriteLine($"ModifyPrivilege failed (SDK error={errCode}). Ignoring and continuing.");
            }

            // Enable user (mandatory)
            if (!EnableUser(dwMachineNumber, dwEnrollNumber, eMachineNumber, backupNumber, 1))
            {
                int errCode;
                GetLastError(dwMachineNumber, out errCode);
                Console.WriteLine($"EnableUser failed for user {dwEnrollNumber} on machine {dwMachineNumber}. SDK Error Code: {errCode}");
                return false;
            }

            // Enable device (mandatory, false then true)
            EnableDevice(dwMachineNumber, 0);
            EnableDevice(dwMachineNumber, 1);
